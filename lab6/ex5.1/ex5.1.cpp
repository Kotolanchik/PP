...
int main(int argc, char* argv[])
{
    ...
    if (proc_rank == 0)
    {
        ...
        double start, end;
        int send_tag;
        start йййййййййййййййййййййй = MPI_Wtime();
        for (int i = 0; i <= N / (proc_num - 1); i++) {
            for (int j = 1; j < proc_num; j++) {
                if (i * (proc_num - 1) + (j - 1) < N) {
                    ...
                    MPI_Send(Matrix[i * (proc_num - 1) + j - 1], M, MPI_LONG_LONG_INT,
                        j, send_tag, MPI_COMM_WORLD);
                }
            }
            for (int j = 1; j < proc_num; j++) {
                if (i * (proc_num - 1) + (j - 1) < N) {
                    MPI_Recv(&LocalSum, 1, MPI_LONG_LONG_INT,
                        j, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                    SumMatrix += LocalSum;
                }
            }
        }
    ...
    else
    {
        ...
        do {
            LocalSum = 0;
            MPI_Recv(Arr, M, MPI_LONG_LONG_INT,
            ...
            MPI_Send(&LocalSum, 1, MPI_LONG_LONG_INT,
                0, 0, MPI_COMM_WORLD);
